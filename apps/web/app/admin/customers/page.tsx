...
import { EllipsisVertical, MessageCircle, Pencil, Plus, UsersRound } from "lucide-react";
...
function imageSource(url: string | null) {
  if (!url) return null;
  return url.startsWith("/") ? `${process.env.NEXT_PUBLIC_API_URL ?? "http://127.0.0.1:8000"}${url}` : url;
}
  ...
  return (
    <main className="app-shell admin-customers-shell">
      <section className="page-heading">
        ...
        <Link className="primary-action" href="/admin/customers/new"><Plus size={18} />Add customer</Link>
      </section>
