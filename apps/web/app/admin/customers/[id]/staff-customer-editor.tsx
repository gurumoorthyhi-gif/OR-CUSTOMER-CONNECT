      ...
      <form className="staff-customer-editor" onSubmit={save}>
        <section className="work-panel staff-customer-editor-card">
          ...
          <div className="staff-editor-grid">
            ...
            <label><span>Customer username</span><input required={isNew} autoComplete="username" value={form.username ?? ""} onChange={(event) => updateField("username", event.target.value)} /></label>
            <label><span>{isNew ? "Customer password" : "New password"}</span><input required={isNew} minLength={6} type="password" autoComplete={isNew ? "new-password" : "new-password"} value={password} onChange={(event) => setPassword(event.target.value)} placeholder={isNew ? "At least 6 characters" : "Leave blank to keep current"} /></label>
