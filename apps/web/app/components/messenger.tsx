...
type MessengerProps = {
  messages: Message[];
  mode: "customer" | "staff";
  initialCustomer?: { id: string; business_name: string; profile_image_url?: string | null; delivery_code?: string } | null;
};
...
export function Messenger({ messages, mode, initialCustomer }: MessengerProps) {
  const apiBase = browserApiBase();
  const [mounted, setMounted] = useState(false);
  const [customerName, setCustomerName] = useState(initialCustomer?.business_name || CUSTOMER_NAME);
  const [customerId, setCustomerId] = useState(initialCustomer?.id || CUSTOMER_ID);
  const [customerAvatarUrl, setCustomerAvatarUrl] = useState<string | null>(initialCustomer?.profile_image_url ?? null);
  const [customerDeliveryCode, setCustomerDeliveryCode] = useState(initialCustomer?.delivery_code || "");
  ...
  const refreshCustomer = useCallback(async () => {
    try {
      const response = await fetch(`${apiBase}${mode === "staff" ? "/api/customers" : "/api/customers/me"}`, { cache: "no-store" });
      if (!response.ok) return;
      const data = await response.json() as { customer?: { id?: string; business_name?: string; profile_image_url?: string | null; delivery_code?: string } } | Array<{ id?: string; business_name?: string; profile_image_url?: string | null; delivery_code?: string }>;
      const customer = Array.isArray(data) ? data[0] : data.customer;
      setCustomerId(customer?.id || CUSTOMER_ID);
      setCustomerName(customer?.business_name || CUSTOMER_NAME);
      setCustomerAvatarUrl(customer?.profile_image_url ?? null);
      setCustomerDeliveryCode(customer?.delivery_code ?? "");
    } catch {
  ...
  useEffect(() => {
    setMounted(true);
    if (mode === "staff") setSidebarWidth(Math.min(380, window.innerWidth));
  ...
  return (
    ...
    >
      ...
      {mode === "staff" ? <Link className="new-customer-button" style={{ left: `${Math.max(12, sidebarWidth - 64)}px` }} href="/admin/customers/new" aria-label="Add new customer" title="Add new customer"><Plus size={22} /></Link> : null}
