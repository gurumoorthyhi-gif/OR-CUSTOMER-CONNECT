...
export function Messenger({ messages, mode }: MessengerProps) {
  ...
  const chatMatches = !chatSearch.trim() || [CUSTOMER_NAME, "OR-TN-0001", messageText(lastMessage ?? {})]
    ...
    const connect = () => {
      ...
      socket.onmessage = (event) => {
        ...
        if (data.type === "message.created" && data.message) {
          ...
          if (incoming && !muted && document.hidden && Notification.permission === "granted") {
            new Notification(mode === "staff" ? CUSTOMER_NAME : "ODD RAVEN", { body: messageText(data.message) });
          }
        }
  ...
  }, [apiBase, fetchMessages, fetchPreference, markRead, mode, muted, upsertMessage]);
  ...
  async function enableNotifications() {
    ...
    showNotice(permission === "granted" ? "Notifications enabled" : "Notifications remain disabled");
    setConversationMenuOpen(false);
  }
  ...
  function selectMedia(files: FileList | null) {
    ...
    const oversized = incoming.find((file) => file.size > 100 * 1024 * 1024);
    if (oversized) {
      showNotice(`${oversized.name} exceeds the 100 MB limit`);
      return;
    }
  ...
  return (
    <main className={`messenger-shell ${mode === "customer" ? "customer-direct-chat" : ""}`}>
      {mode === "staff" ? (
          ...
          <div className="messenger-sidebar-header">
            ...
            {conversationMenuOpen ? (
              <div className="conversation-menu">
                ...
                <button type="button" onClick={enableNotifications}><Bell size={17} />Enable notifications</button>
              </div>
            ) : null}
          </div>
          <label className="chat-search">
            <Search size={16} />
            <input value={chatSearch} onChange={(event) => setChatSearch(event.target.value)} placeholder="Search conversations" />
          </label>
          <div className="chat-list">
            ...
            {chatMatches && (!preference.is_archived || showArchived) ? (
              <button className="chat-tile selected" type="button" onClick={() => preference.marked_unread && updateConversationPreference("mark_read")}>
                ...
                <span>
                  <strong>{preference.is_pinned ? <Pin size={13} /> : null}{CUSTOMER_NAME}</strong>
      ...
      <section className="conversation">
        <header className="conversation-header">
          <button className="profile-button" type="button" onClick={() => mode === "staff" && setInfoOpen(true)} aria-label="Conversation information">
            <div className="avatar">{mode === "customer" ? "OR" : "SP"}</div>
            <span>
              <strong>{mode === "customer" ? "ODD RAVEN" : CUSTOMER_NAME}</strong>
          ...
          <div className="conversation-actions">
            <label className="message-search" aria-label="Search messages">
              ...
              />
              {messageSearch ? <small>{searchResults.length ? `${searchIndex + 1}/${searchResults.length}` : "0/0"}</small> : null}
              {messageSearch ? (
                <>
        ...
        <div className="conversation-body" ref={bodyRef} onScroll={(event) => {
          ...
          {visibleMessages.map((message, index) => {
            ...
            const highlighted = searchResults[searchIndex]?.id === message.id;
              ...
              <div key={messageKey}>
                ...
                <div className="message-row-wrap">
                  <article id={message.id ? `message-${message.id}` : undefined} className={`bubble ${own ? "own" : "theirs"} ${deleted ? "deleted" : ""} ${highlighted ? "search-highlight" : ""}`}>
                    <button className="message-menu-trigger" type="button" aria-label="Message actions" onClick={() => setMenuMessageId((current) => current === messageKey ? null : messageKey)}>
                      <ChevronDown size={16} />
                    </button>
                    {menuMessageId === messageKey ? (
                      <div className={`message-action-menu ${own ? "align-right" : ""}`}>
                    ...
                    {message.reply_to_body ? (
                      <button className="message-quote" type="button" onClick={() => document.getElementById(`message-${message.reply_to_message_id}`)?.scrollIntoView({ block: "center", behavior: "smooth" })}>
                        <strong>{message.reply_to_sender_type === "staff" ? "ODD RAVEN" : CUSTOMER_NAME}</strong>
        ...
        <form className="conversation-compose" onSubmit={sendMessage}>
          {replyTarget ? (
            <div className="reply-preview">
              <div>
                <strong>{isOwnMessage(replyTarget, mode) ? "You" : mode === "customer" ? "ODD RAVEN" : CUSTOMER_NAME}</strong>
          ...
          {recordingState !== "idle" ? (
            ...
            <div className="compose-row">
              <button type="button" aria-label="Emoji" onClick={() => setEmojiOpen((open) => !open)}><Smile size={22} /></button>
              ...
              <textarea ref={textareaRef} rows={1} value={draft} onChange={(event) => handleDraftChange(event.target.value)} onKeyDown={handleComposerKey} placeholder="Type a message" />
      ...
      {mode === "staff" && infoOpen ? (
        <aside className="chat-info-panel">
          ...
          <div className="info-profile"><div className="avatar large">SP</div><h2>{CUSTOMER_NAME}</h2><span>OR-TN-0001</span></div>
