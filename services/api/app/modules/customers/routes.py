...
def serialize_customer(customer: Customer) -> dict:
    return {
        ...
        "has_login": bool(customer.username and customer.password_hash),
    }
...
def apply_login_payload(customer: Customer, payload: dict, db: Session, *, required: bool) -> None:
    ...
    if len(username) < 3 or not re.fullmatch(r"[a-z0-9._-]+", username):
        raise HTTPException(status_code=422, detail="Username must contain at least 3 letters, numbers, dots, underscores, or hyphens")
    if len(password) < 6:
        raise HTTPException(status_code=422, detail="Password must be at least 6 characters")
    existing = db.scalar(select(Customer).where(Customer.username == username, Customer.id != customer.id))
    if existing:
...
def customer_from_session(request: Request, db: Session) -> Customer | None:
    ...
    try:
        ...
        return None
    return db.scalar(select(Customer).where(Customer.public_id == public_id)) if public_id else None


@router.post("/login")
def customer_login(payload: dict, response: Response, db: DbSession) -> dict:
    username = str(payload.get("username") or "").strip().lower()
    password = str(payload.get("password") or "")
    ...
    return {"status": "authenticated", "customer": serialize_customer(customer)}


@router.post("/logout")
def customer_logout(response: Response) -> dict[str, str]:
...
def create_customer(payload: dict, db: DbSession) -> dict:
    ...
    apply_login_payload(customer, payload, db, required=True)
...
@router.get("/me")
def get_my_profile(request: Request, db: DbSession) -> dict:
...
@router.patch("/me")
def update_my_profile(request: Request, payload: dict, db: DbSession) -> dict:
    customer = customer_from_session(request, db)
    if not customer:
        raise HTTPException(status_code=401, detail="Customer login required")
...
def update_customer_by_staff(customer_id: str, payload: dict, db: DbSession) -> dict:
    ...
    apply_profile_payload(customer, payload)
    apply_login_payload(customer, payload, db, required=False)
    customer.profile_locked = True
