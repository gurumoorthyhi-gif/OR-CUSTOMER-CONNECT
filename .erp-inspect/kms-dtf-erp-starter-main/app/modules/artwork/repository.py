...
class ArtworkRepository:
    ...
    def _scoped(self, statement):
        if self.organization_id is not None:
            statement = statement.where(Artwork.organization_id == self.organization_id)
        return statement
    ...
    def add_version(self, artwork_id: int, stored: StoredArtworkFile, notes: str) -> Artwork | None:
        with session_scope(self.factory) as session:
            artwork = session.scalar(
                self._scoped(select(Artwork).where(Artwork.id == artwork_id))
    ...
    def get_version(self, version_id: int) -> ArtworkVersion | None:
        with session_scope(self.factory) as session:
            statement = (
                select(ArtworkVersion)
                .join(Artwork)
                .where(ArtworkVersion.id == version_id)
                .options(selectinload(ArtworkVersion.approvals))
            )
            if self.organization_id is not None:
                statement = statement.where(Artwork.organization_id == self.organization_id)
    ...
    def add_approval(
        ...
        with session_scope(self.factory) as session:
            statement = select(ArtworkVersion).join(Artwork).where(ArtworkVersion.id == version_id)
            if self.organization_id is not None:
                statement = statement.where(Artwork.organization_id == self.organization_id)
    ...
    def _version(number: int, stored: StoredArtworkFile, notes: str) -> ArtworkVersion:
        return ArtworkVersion(
            ...
            preview_cloud_file_id=stored.preview_cloud_file_id,
