...
class ArtworkVersion(Base):
    ...
    cloud_file_id: Mapped[int | None] = mapped_column(
        ForeignKey("cloud_files.id", ondelete="RESTRICT"), nullable=True, index=True
    )
    preview_cloud_file_id: Mapped[int | None] = mapped_column(
        ForeignKey("cloud_files.id", ondelete="SET NULL"), nullable=True, index=True
    )
