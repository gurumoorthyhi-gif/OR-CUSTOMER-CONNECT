...
class CustomerRepository:
    ...
    def _scoped(self, statement):
        if self._organization_id is not None:
            statement = statement.where(Customer.organization_id == self._organization_id)
        return statement
    ...
    def get(self, customer_id: int) -> Customer | None:
        with session_scope(self._session_factory) as session:
            statement = self._scoped(select(Customer).where(Customer.id == customer_id)).options(
                *self._load_options()
            )
    ...
    def get_by_code(self, code: str) -> Customer | None:
        with session_scope(self._session_factory) as session:
            return session.scalar(self._scoped(select(Customer).where(Customer.code == code)))
    ...
    def next_code(self, delivery_type: str) -> str:
        ...
        with session_scope(self._session_factory) as session:
            ...
            codes = session.scalars(statement)
    ...
    def search(
        ...
        *,
        ...
        with session_scope(self._session_factory) as session:
            statement = self._scoped(select(Customer)).options(*self._load_options())
            if query:
                pattern = f"%{query}%"
    ...
    def update(self, customer_id: int, data: CustomerInput) -> Customer | None:
        with session_scope(self._session_factory) as session:
            customer = session.scalar(
                self._scoped(select(Customer).where(Customer.id == customer_id)).options(
                    selectinload(Customer.addresses)
                )
            )
            if customer is None:
    ...
    def set_storage(
        ...
        *,
        ...
        with session_scope(self._session_factory) as session:
            customer = session.scalar(
                self._scoped(select(Customer).where(Customer.id == customer_id))
            )
            if customer is None:
                return None
