...
class CloudStorageService:
    ...
    def ensure_folder(self, prefix: str, *, auto_sync: bool = True) -> CloudFile:
        ...
        with session_scope(self.factory) as session:
            statement = select(CloudFile).where(CloudFile.object_key == key)
            if self.organization_id is not None:
                statement = statement.where(CloudFile.organization_id == self.organization_id)
    ...
    def get(self, record_id: int) -> CloudFile:
        with session_scope(self.factory) as session:
            statement = select(CloudFile).where(CloudFile.id == record_id)
            if self.organization_id is not None:
                statement = statement.where(CloudFile.organization_id == self.organization_id)
