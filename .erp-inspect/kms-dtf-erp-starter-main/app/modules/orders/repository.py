...
class OrderRepository:
    ...
    def _scoped(self, statement):
        if self.organization_id is not None:
            statement = statement.where(Order.organization_id == self.organization_id)
        return statement
    ...
    def list(self, query: str = "") -> list[Order]:
        ...
        with session_scope(self.factory) as session:
            statement = self._scoped(select(Order)).join(Order.customer).options(
    ...
    def get(self, order_id: int) -> Order | None:
        with session_scope(self.factory) as session:
            return session.scalar(
                self._scoped(select(Order).where(Order.id == order_id))
                .options(
                    selectinload(Order.customer),
    ...
    def change_status(
        ...
        *,
        ...
        with session_scope(self.factory) as session:
            order = session.scalar(self._scoped(select(Order).where(Order.id == order_id)))
            if order is None:
                return None
