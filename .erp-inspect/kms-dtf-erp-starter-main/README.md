...
The browser application is being developed as a separate runtime under `api/` so
the existing desktop ERP remains usable during migration. See
[`docs/WEB_MIGRATION_PLAN.md`](docs/WEB_MIGRATION_PLAN.md) for the ordered rollout,
local API command, safety gates, and eventual Render entry point.

After starting the API, open `http://127.0.0.1:8000/` for the responsive browser
interface. Interactive API documentation remains available at `/docs` outside
production.
